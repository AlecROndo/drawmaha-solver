...
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}p{6.9cm}p{2.5cm}p{3.7cm}@{}}
\toprule
Case & Handling & Where \\
\midrule
One-action ledger (degenerate game) & raises & \texttt{regret\_matching.py:31} \\
All regrets negative (no positive signal) & uniform fallback & \texttt{regret\_matching.py:40} \\
Player given a non-distribution strategy & raises & \texttt{players.py:37} \\
Garbage terminal input & reprompts & \texttt{players.py:79} \\
Quit before playing any round & no summary & \texttt{play.py:38} \\
Analysis run shorter than the 5{,}000-iteration plot window & window clamps & \texttt{analysis.py:156} \\
...
Gate & Status & Note \\
\midrule
Branch / commits & \texttt{alec/rung0-rps} & \texttt{983066b}, \texttt{623b96b}, \texttt{e2e9022} --- \href{https://github.com/AlecROndo/drawmaha-solver/pull/1}{PR \#1} \\
