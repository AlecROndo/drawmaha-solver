...
from drawmaha_solver.kuhn.game import DEAL_PROBABILITY, DEALS, KuhnState
...
def run_iteration(table: InfoSetTable) -> None:
    ...
    """
    for deal in DEALS:
        walk(KuhnState(cards=deal), table, (1.0, 1.0))
...
def walk(
    state: KuhnState, table: InfoSetTable, reach: tuple[float, float]
) -> tuple[float, float]:
    ...
    # played with. Only the acting player's own reach advances here.
    children = [
        walk(state.apply(action), table, _advance(reach, player, sigma[action]))
        for action in state.legal_actions()
    ]

    # The utilities the ledger wants are this player's column of the children's
    # values: "what is each action worth to me from here".
    utilities = np.array([child[player] for child in children])
    ledger.update(
        utilities,
        # Chance rides with the opponent because the deck is not something the
        # player controls, and it is absent from the strategy weight entirely.
